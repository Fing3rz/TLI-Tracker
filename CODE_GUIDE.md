...
- Never overwrite `full_table.json` names or prices without explicit user intent. `apply_local_overrides()` only _adds missing IDs_ and _fills missing types/names_ when the existing name is empty.
